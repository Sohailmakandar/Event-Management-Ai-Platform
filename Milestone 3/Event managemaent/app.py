...
@app.route('/')
def index():
    events = Event.query.filter(Event.status != 'completed').order_by(Event.date).all()
...
def contact():
    if request.method == 'POST':
        ...
        if name and email and message:
            flash(f"Thanks {name}! Your message has been received. We'll get back to you at {email} within 24 hours.", 'success')
        else:
